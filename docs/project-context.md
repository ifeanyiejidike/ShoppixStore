...
PROJECT: Shoppix
STAGE: active build
LAST UPDATED: 2026-08-19 (0 days ago) — Full clean-room verification of both apps: found and fixed a production-build-breaking font dependency, 15 npm vulnerabilities, and 56 pip vulnerabilities (including an unpatched Django branch) — zero vulnerabilities remain on both sides, full purchase flow re-verified end-to-end on the updated stack
LIVE BLOCKER: none
NEXT ACTION: A real visual browser QA pass is still owed — this sandbox cannot run headless Chromium, so all verification so far has been code/HTTP/build-based, not visual. Do a manual pass in an actual browser across breakpoints before considering the UI audit fully closed — owner: user
```
...
- **Palette**: Ink `#10241C` (near-black green, text/dark surfaces), Canvas `#F6F5EF` (warm paper background), Marigold `#E8A33D` (primary accent/CTAs), Jade `#12684A` (secondary accent/vendor context), Coral (destructive/urgency — darkened from an initial 53% to 44% lightness this session after a computed WCAG contrast audit found the original failed AA for text; see Section 11 changelog).
- **Type**: Fraunces (display/headings, used italic), Inter (body/UI), IBM Plex Mono (prices, order references — a deliberate "receipt/ledger" register for commerce data).
- **Signature element**: a die-cut "price tag" notch (`.price-tag` utility in globals.css) on product cards and hero decorative elements — a literal market-price-tag motif tying back to the brand's Nigerian-marketplace positioning, not a generic card shadow.
- **Component states**: Interactive elements have default/hover/active/focus/disabled/loading states via Tailwind + Radix primitives; `:focus-visible` has a global visible outline (marigold, 2px) rather than relying on browser defaults.
...
- **Accessibility**: WCAG 2.1 AA is the active floor, not just an aspiration — a full audit pass this session found and fixed real AA violations (contrast, invalid nested-interactive HTML, missing ARIA group roles, missing accessible names on search inputs). See Section 11's 2026-08-19 entries for the specific findings and fixes.
- **Four-part design justification test**: logical (serves content/function), strategic (serves the business goal), emotional (fits brand/audience), accessible — applied to the price-tag/market-signage direction (functional: clearly separates price from other card info; strategic: differentiates from generic Jumia/Amazon-clone aesthetics; emotional: grounds the brand in the physical Nigerian-market experience it's digitizing; accessible: verified contrast, not just chosen for looks).

**Known gap**: everything above has been verified by a human-inspectable rendered HTML/CSS/JS review, `tsc`, `eslint`, and live HTTP checks against the running dev server — but not by an actual person looking at it in a real browser across real device widths. This sandbox cannot run headless Chromium (network-restricted). Treat the UI as code-verified, not visually-verified, until someone does that pass.
